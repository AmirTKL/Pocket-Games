...
import { times, remove, addWithCharCode, clamp, range, wrap } from "./util";
import {
  get as getButton,
...
export function frameState(frameState: any) {
  if (isWaitingRewind) {
    window.rs = replay.getLastFrameState(random);
    window.bs = window.rs.baseState;
    window.score = window.bs.score;
    window.ticks = window.bs.ticks;
    return cloneDeep(window.rs.gameState);
  } else if (isRewinding) {
    window.rs = replay.rewind(random);
    window.bs = window.rs.baseState;
    window.score = window.bs.score;
    window.ticks = window.bs.ticks;
    return window.rs.gameState;
  } else if (window.isReplaying) {
    const rs = replay.getFrameStateForReplay();
    return rs.gameState;
  } else if (state === "inGame") {
    const baseState = { score: window.score, ticks: window.ticks };
...
export function rewind() {
  ...
  if (!window.isReplaying && currentOptions.isRewindEnabled) {
    initRewind();
  } else {
    end();
  }
...
function _update() {
  window.df = window.difficulty = window.ticks / 3600 + 1;
  window.tc = window.ticks;
  window.prevScore = window.score;
  window.prevTime = window.time;
  window.sc = window.score;
  const prevSc = window.sc;
  window.inp = {
    p: window.input.pos,
    ip: window.input.isPressed,
    ijp: window.input.isJustPressed,
    ijr: window.input.isJustReleased,
  };
  collision.clear();
  updateFunc[state]();
  ...
  window.ticks++;
  if (window.isReplaying) {
    window.score = window.prevScore;
    window.time = window.prevTime;
  } else if (window.sc !== prevSc) {
    window.score = window.sc;
  }
}
...
function initInGame() {
  ...
  window.ticks = -1;
  _particle.init();
  const s = Math.floor(window.score);
  if (s > hiScore) {
    hiScore = s;
  }
  if (currentOptions.isShowingTime && window.time != null) {
    if (fastestTime == null || fastestTime > window.time) {
      fastestTime = window.time;
    }
  }
  window.score = 0;
  window.time = 0;
...
function updateInGame() {
  ...
  if (currentOptions.isReplayEnabled || currentOptions.isRewindEnabled) {
    replay.recordInput({
      pos: vec(window.input.pos),
      isPressed: window.input.isPressed,
      isJustPressed: window.input.isJustPressed,
      isJustReleased: window.input.isJustReleased,
    });
  }
...
function updateGameOver() {
  if (window.ticks === 0 && !view.theme.isUsingPixi) {
    drawGameOver();
  }
  if ((window.isReplaying || window.ticks > 20) && window.input.isJustPressed) {
    stopRecorder();
    initInGame();
  } else if (
    window.ticks === (currentOptions.isReplayEnabled ? 120 : 300) &&
    !isNoTitle
  ) {
    stopRecorder();
    initTitle();
...
function stopRecorder() {
  if (!currentOptions.isRecording || window.isReplaying) {
    return;
  }
  stopRecording();
}
...
function updateRewind() {
  ...
  if (isRewinding) {
    drawButton(rewindButton);
    if (replay.isFrameStateEmpty() || !window.input.isPressed) {
      stopRewind();
    }
  } else {
...
function drawScoreOrTime() {
  if (currentOptions.isShowingTime) {
    ...
    print(`${Math.floor(window.score)}`, 3, 3, {
      isSmallText: currentOptions.isUsingSmallText,
...
export function addAllWindow() {
  window.ticks = ticks;
  window.difficulty = difficulty;
  window.score = score;
  window.time = time;
  window.isReplaying = isReplaying;
  window.df = df;
  window.tc = tc;
  window.sc = sc;
  window.inp = inp;
  ...
  window.addWithCharCode = addWithCharCode;
  window.arc = arc;
  window.atan2 = atan2;
  window.bar = bar;
  window.bl = bl;
  ...
  window.ex = ex;
  window.floor = floor;
  window.frameState = frameState;
  window.getButton = get;
  window.gr = gr;
  window.ht = ht;
  window.init = init;
  window.input = input;
  ...
  window.max = max;
  window.min = min;
  window.minifyReplaces = minifyReplaces;
